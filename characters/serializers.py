...
class CharacterSerializer(serializers.ModelSerializer):
    ...
    def validate_statisctic(self, value):
        if value is not None and self.instance is not None and Character.objects.filter(statisctic=value).exclude(pk=self.instance.pk).exists():
            raise serializers.ValidationError("This statistics has already been assigned to another Character.")

        if self.instance is None and Character.objects.filter(statisctic = value).exists():
            raise serializers.ValidationError("This statistics has already been assigned to another Character.")
